 ...
 *
 * Implementation follows as close as possible to the original. Only public API "generateKeyBetween()" has been
 * tightened types and always returns a key or throws an exception.
 *
 * @internal
 */
class FractionalIndexing
{
    ...
    public static function generateKeyBetween(?string $a, ?string $b, string $digits = self::BASE_62_DIGITS): string
    {
        ...
        if ($a === null) {
            ...
            $decremented = self::decrementInteger($ib, $digits);
            if ($decremented === null) {
                throw new \RuntimeException('Cannot generate an order key below the smallest integer: ' . $b, 1790263266);
            }
            return strcmp($ib, $b) < 0 ? $ib : $decremented;
        }
