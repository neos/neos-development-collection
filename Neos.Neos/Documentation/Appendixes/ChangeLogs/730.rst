...
`FEATURE: Change wallpaper for login <https://github.com/neos/neos-development-collection/pull/3533>`_
------------------------------------------------------------------------------------------------------

**What I did**
Replaces the JPG and WebP file of the wallpaper.

**How I did it**
Used imageOptim to minimize the image from @kdambekalns with Gruetzli and convert the image also to a WebP.

**How to verify it**
Check the wonderful new login screen.
Thanks to @kdambekalns for providing such a nice picture from Saxony-Anhalt (Germany) ❤️ 

* Resolves: `#3522 <https://github.com/neos/neos-development-collection/issues/3522>`_

!`Screenshot 2021-12-07 at 00 54 19 <https://user-images.githubusercontent.com/1014126/144941468-55cb4a29-f073-4c5c-b9cd-7ce90a8149f5.png>`_
...
- [x] configure package subsplit https://github.com/neos/slicer/pull/13
- [x] register package via packagist
- [x] adjust package to neos-dev-distribution (post-create-project-script, set-dependencies.sh) https://github.com/neos/neos-development-distribution/pull/75
- [x] adjust neos-base-distributions (require + post-create-project-script) https://github.com/neos/neos-base-distribution/pull/54
...
**What I did**
before the attributes are handed into the big loop and converted to fusion, some keys are modified.
Each attribute key string that ends with either ``@if`` or ``@process`` will get an auto generated key.

**Improvements i did not do yet.**
I think the function ``generatePathForShorthandFusionMetaPath`` could be integrated in the for loop succeeding it (we would save one loop then) but i did not want to create spaghetti yet. (The generation relies on a ``global`` index list wich will decide the next number - easier would be a random number or a not path aware incrementing count. (a simple i++))

btw watch out:
The global editorconfig from neos removes wanted whitespaces from the AfxServiceTest.php...
...
- AFX: allow quoted attribute identifiers.
- added some tests.
