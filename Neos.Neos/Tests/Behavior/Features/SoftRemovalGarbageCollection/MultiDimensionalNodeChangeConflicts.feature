Feature: Tests for soft removal garbage collection with impending conflicts targeting the same node
  ...
  Scenario: Succeeding changes to one node in multiple dimensions will avoid garbage collection
    ...
    # the impending conflict is extended
    Then I expect the following hard removal conflicts to be impending:
      | nodeAggregateId | dimensionSpacePoints                                                   |
