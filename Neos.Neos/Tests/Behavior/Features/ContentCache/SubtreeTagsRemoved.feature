...
Feature: Tests for the ContentCacheFlusher and cache flushing on node and nodetype specific tags
  ...
  Background:
    ...
    And the following CreateNodeAggregateWithNode commands are executed:
      ...
      | a4              | a                     | Neos.Neos:Test.DocumentType2 | {"uriPathSegment": "a4", "title": "Node a4"}         | a4       |

    And the following TagSubtree commands are executed:
      | nodeAggregateId | workspaceName | coveredDimensionSpacePoint | nodeVariantSelectionStrategy | tag      |
      | a1-1            | live          | {}                         | allSpecializations           | disabled |
      | a1-1-1          | live          | {}                         | allSpecializations           | disabled |
      | a3              | live          | {}                         | allSpecializations           | disabled |
      | a4              | live          | {}                         | allSpecializations           | disabled |
  ...
  Scenario: ContentCache gets not flushed when a subtree tag to another node has been removed
  ...
  Scenario: ContentCache gets flushed when a subtree tag to another node has been removed that is added as nodeTagForIdentifier
    Given I have Fusion content cache enabled
    And the Fusion context node is a1
  ...
  Scenario: ContentCache gets flushed when a subtree tag to a node has been removed by NodeType name
    Given I have Fusion content cache enabled
  ...
  Scenario: ContentCache gets flushed when a subtree tag to a node has been removed of a descendant node
    Given I have Fusion content cache enabled
    And the Fusion context node is "a1"
    And I execute the following Fusion code:
    """fusion
  ...
  Scenario: ContentCache gets flushed when a subtree tag to a node has been removed of a descendant node (2 levels)
    Given I have Fusion content cache enabled
    And the Fusion context node is "a1"
    And I execute the following Fusion code:
    """fusion
