...
Feature: Tests for the ContentCacheFlusher and cache flushing on node and nodetype specific tags
  ...
  Scenario: ContentCache gets not flushed when a subtree tag to another node been added
  ...
  Scenario: ContentCache gets flushed when a subtree tag to another node has been added that is added as nodeTagForIdentifier
    Given I have Fusion content cache enabled
    And the Fusion context node is a1
  ...
  Scenario: ContentCache gets flushed when a subtree tag to a node has been added by NodeType name
    Given I have Fusion content cache enabled
  ...
  Scenario: ContentCache gets flushed when a subtree tag to a node has been added of a descendant node
    Given I have Fusion content cache enabled
    And the Fusion context node is "a1"
    And I execute the following Fusion code:
    """fusion
  ...
  Scenario: ContentCache gets flushed when a subtree tag to a node has been added of a descendant node (2 levels)
    Given I have Fusion content cache enabled
    And the Fusion context node is "a1"
    And I execute the following Fusion code:
    """fusion
