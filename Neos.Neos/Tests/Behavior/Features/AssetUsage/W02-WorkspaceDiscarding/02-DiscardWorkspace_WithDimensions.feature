...
Feature: Discard workspace with dimensions
  ...
  Scenario: Discard user workspace after change to an existing asseet usage of a property
    ...
    And the command SetNodeProperties is executed with payload:
      ...
      | propertyValues            | {"asset": "Asset:asset-2", "assets": ["Asset:asset-2", "Asset:asset-1"]} |

    And I expect the AssetUsageService to have the following AssetUsages:
      | assetId | nodeAggregateId        | propertyName | workspaceName  | originDimensionSpacePoint |
      | asset-1 | sir-david-nodenborough | asset        | live           | {"language": "de"}        |
      | asset-2 | sir-david-nodenborough | asset        | user-workspace | {"language": "de"}        |
      | asset-2 | sir-david-nodenborough | assets       | user-workspace | {"language": "de"}        |
      | asset-1 | sir-david-nodenborough | assets       | user-workspace | {"language": "de"}        |
