...
namespace Neos\ContentRepository\BehavioralTests\Tests\Parallel\ParallelSubtreeTaggingInWorkspace;
...
/**
 * See https://github.com/neos/neos-development-collection/pull/5779#issuecomment-4243116784
 */
class ParallelSubtreeTaggingInWorkspaceTest extends AbstractParallelTestCase
{
    ...
    public function setUp(): void
    {
        ...
        $contentRepository->handle(CreateNodeAggregateWithNode::create(
            ...
            NodeTypeName::fromString('Neos.ContentRepository.Testing:Site'),
            $origin,
            NodeAggregateId::fromString('lady-eleonode-rootford'),
     ...
     * @group parallel
     */
    public function whileANodesAreDisabledOnLive(): void
    {
        ...
        try {
            for ($i = 0; $i <= 2000; $i++) {
                ...
                $node = $subgraph->findNodeById(NodeAggregateId::fromString('nody-mc-nodeface-0'));

                try {
                    if ($node->tags->contain(SubtreeTag::fromString('disabled'))) {
                        $this->log('untag');
                        $this->contentRepository->handle(UntagSubtree::create(
                            WorkspaceName::forLive(),
                            NodeAggregateId::fromString('nody-mc-nodeface-0'),
                    ...
                    $this->log(sprintf('Got exception %s: %s', self::shortClassName($thrownException::class), $thrownException->getMessage()));
                    Assert::assertContains($thrownException::class, [
                        SubtreeIsNotTagged::class,
                        SubtreeIsAlreadyTagged::class,
     ...
     * @group parallel
     */
    public function thenConcurrentDisablingLeadsToException(): void
    {
