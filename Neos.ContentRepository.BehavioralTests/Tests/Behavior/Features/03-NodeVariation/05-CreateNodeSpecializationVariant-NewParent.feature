...
Feature: Create node specialization
  ...
  * after one of its new siblings, which is not the last
  ...
  Scenario: Create specialization variant to a new parent before the first of its new siblings
    ...
    Then I expect exactly 23 events to be published on stream "ContentStream:cs-identifier"
    And event at index 20 is of type "NodeSpecializationVariantWasCreated" with payload:
      ...
      | parentNodeAggregateId  | "sir-david-nodenborough"                                                                                                                                                |
    And event at index 21 is of type "NodeSpecializationVariantWasCreated" with payload:
      | Key                    | Expected                                                                                                                                                                        |
      | contentStreamId        | "cs-identifier"                                                                                                                                                                 |
    ...
    And event at index 22 is of type "NodeSpecializationVariantWasCreated" with payload:
      | Key                    | Expected                                                                                                                                |
      | contentStreamId        | "cs-identifier"                                                                                                                         |
    ...
    Then I expect the graph projection to consist of exactly 22 nodes
    ...
    Then I expect the subgraph projection to consist of exactly 19 nodes
    ...
    And the subtree for node aggregate "lady-eleonode-rootford" with node types "" and 4 levels deep should be:
      ...
      | 3     | eldest-nodewyn-tetherton   |
      | 4     | eldest-nodimer-tetherton   |
      | 2     | elder-mc-nodeface          |
      | 3     | elder-nodewyn-tetherton    |
      | 4     | elder-nodimer-tetherton    |
      | 2     | younger-mc-nodeface        |
      | 3     | younger-nodewyn-tetherton  |
      | 4     | younger-nodimer-tetherton  |
      | 2     | youngest-mc-nodeface       |
      | 3     | youngest-nodewyn-tetherton |
      | 4     | youngest-nodimer-tetherton |
