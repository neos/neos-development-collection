Feature: Untag nodes after moving their children in or out
  ...
  - gatherAll
  strategy and then untag the parent in
  - allSpecializations
  - allVariants
  ...
  Background:
    ...
    And I set up the edge case workspace tree and the following additional commands:
      ...
      | TagSubtree                      | {"workspaceName": "live", "nodeAggregateId": "sir-david-nodenborough", "coveredDimensionSpacePoint": {"example": "source"}, "nodeVariantSelectionStrategy": "allVariants", "tag": "my-tag"}                                                                              |
      | TagSubtree                      | {"workspaceName": "live", "nodeAggregateId": "sir-nodeward-nodington-iii", "coveredDimensionSpacePoint": {"example": "source"}, "nodeVariantSelectionStrategy": "allVariants", "tag": "my-tag"}                                                                          |

  Scenario: Publish moving a child node out via scatter strategy and untagging the parent and all its specializations
  ...
  Scenario: Partially publish moving a child node out via scatter strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node out via scatter strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node out via scatter strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node out via gatherSpecializations strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node out via gatherSpecializations strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node out via gatherSpecializations strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node out via gatherSpecializations strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node in via scatter strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node in via scatter strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node in via scatter strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node in via scatter strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node in via gatherSpecializations strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node in via gatherSpecializations strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node in via gatherSpecializations strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node in via gatherSpecializations strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node in via gatherAll strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node in via gatherAll strategy and untagging the parent and all its specializations
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Publish moving a child node in via gatherAll strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
  ...
  Scenario: Partially publish moving a child node in via gatherAll strategy and untagging the parent and all its variants
    When the command MoveNodeAggregate is executed with payload:
      | Key                          | Value                        |
      | workspaceName                | "local"                      |
      | dimensionSpacePoint          | {"example": "source"}        |
