...
{
    /**
     * @var array<\Neos\EventSourcedContentRepository\Domain\Model\NodeInterface>
     */
     ...
     * @throws \Exception
     * @throws \Neos\EventSourcedContentRepository\Exception\NodeExistsException
    ...
    public function iHaveTheFollowingWorkspaces($table)
    {
        if ($this->isolated === true) {
            ...
            $rows = $table->getHash();
            $workspaceRepository = $this->getObjectManager()->get(\Neos\EventSourcedContentRepository\Domain\Repository\WorkspaceRepository::class);
            foreach ($rows as $row) {
                $name = $row['Name'];
    ...
    public function iDiscardTheWorkspace($workspaceName)
    {
        if ($this->isolated === true) {
            ...
            $publishingService = $this->getObjectManager()->get(\Neos\EventSourcedContentRepository\Domain\Service\PublishingServiceInterface::class);
     ...
     *
     * @return \Neos\EventSourcedContentRepository\Domain\Model\NodeInterface
     */
    ...
    public function iShouldBeAbleToCreateAChildNodeOfType($not, $nodeTypeName)
    {
        if ($this->isolated === true) {
            ...
            if (empty($not)) {
                ...
                } catch (\Neos\EventSourcedContentRepository\Exception\NodeConstraintException $nodeConstraintExceptio) {
                    // Expected exception
                }
            }
        }
     ...
     * @return \Neos\EventSourcedContentRepository\Domain\Service\Context
     * @throws \Exception
     * @throws Dimension\Exception\MissingContentDimensionDefaultValueException
     */
    protected function getContextForProperties(array $humanReadableContextProperties, $addDimensionDefaults = false)
    {
        if ($this->isolated === true) {
            ...
            /** @var \Neos\EventSourcedContentRepository\Domain\Service\ContextFactoryInterface $contextFactory */
            $contextFactory = $this->getObjectManager()->get(\Neos\EventSourcedContentRepository\Domain\Service\ContextFactoryInterface::class);
