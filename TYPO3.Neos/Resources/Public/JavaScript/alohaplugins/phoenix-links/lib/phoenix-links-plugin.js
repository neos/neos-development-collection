...
function(Plugin, block, BlockManager, require) {
	...
	var PhoenixLinksPlugin = Plugin.create('phoenix-links', {
		dependencies: ['block'],
		...
		init: function() {
			...
			new ( Aloha.AbstractRepository.extend( {
				...
				_constructor: function () {
					this._super( 'phoenixLinksRepository' );
				},
				...
				query: function ( p, callback ) {
					var that = this;
					TYPO3_TYPO3_Service_ExtDirect_V1_Controller_NodeController.searchPage(p.queryString, function(result) {
				...
				getObjectById: function ( itemId, callback ) {
					var that = this;
					TYPO3_TYPO3_Service_ExtDirect_V1_Controller_NodeController.getPageByNodePath(itemId, function(result) {
						result = result.node;
						result.repositoryId = 'phoenixLinksRepository';
