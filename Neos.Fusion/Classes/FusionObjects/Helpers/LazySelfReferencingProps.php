...
/** @Flow\Proxy(false) */
final class LazySelfReferencingProps extends AbstractLazyProps
{
    ...
    public function __construct(
        ...
        private string $selfReferencingPath
    ) {
        parent::__construct($keys);
        $this->effectiveContext[$this->selfReferencingPath] = $this;
    }
    ...
    public function offsetGet($path): mixed
    {
        if (!array_key_exists($path, $this->valueCache)) {
            if (isset($this->currentlyEvaluatingIndexes[$path])) {
                throw new \RuntimeException('Circular reference detected while evaluating: "' . $this->selfReferencingPath . '.' . $path . '"', 1669654158);
            }
