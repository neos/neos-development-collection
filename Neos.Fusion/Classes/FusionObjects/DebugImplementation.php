...
class DebugImplementation extends ArrayImplementation
{
    ...
    protected function dumpToConsole($variable, $title, $separator, $maxDepth)
    {
        ...
        return ('<script>let bla = ' . $consoleDump . '; console.log(bla);</script>');
    }
    ...
    protected function renderConsoleDump($variable, $depth, $name, $separator, $maxDepth)
    {
        ...
        if (is_string($variable)) {
            ...
            $result[$name . $separator . 'string'] = $croppedValue;
        } elseif (is_numeric($variable)) {
            $result[$name . $separator . gettype($variable)] = $variable;
        } elseif (is_array($variable)) {
            $result[$name . $separator . 'array'] = $this->renderArray($variable, $depth, $separator, $maxDepth);
        } elseif (is_object($variable)) {
            $result[$name . $separator . gettype($variable)] =
                $this->renderObject($variable, $depth, $separator, $maxDepth);
        } elseif (is_bool($variable)) {
            $result[$name . $separator . 'boolean'] = $variable;
        } elseif (is_null($variable) || is_resource($variable)) {
            $result[$name . $separator . gettype($variable)] = gettype($variable);
        } else {
            $result[$name . $separator . gettype($variable)] = 'UNHANDLED TYPE';
        }
        return $result;
    }
