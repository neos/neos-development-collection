...
use Neos\Fusion\Core\Cache\CompileTimeParserCache;
        ...
        if (!$context->isProduction()) {
            $dispatcher->connect(Sequence::class, 'afterInvokeStep', function ($step) use ($bootstrap, $dispatcher) {
                ...
                if ($step->getIdentifier() === 'neos.flow:cachemanagement') {
                    ...
                    $parsePartialsCache = $cacheManager->getCache('Neos_Fusion_ParsePartials');
                    // We cannot use the `ParserCache`, not even with a static method, because once a class is registered at boot time,
                    // It can't be unregistered, so we can never use the Proxy version with DI.
                    $compileTimeParserCache = new CompileTimeParserCache($parsePartialsCache);
                    $dispatcher->connect(FileMonitor::class, 'filesHaveChanged', $compileTimeParserCache, 'flushParsePartialsOnFileChanges');
                }
            });
        }
    }
