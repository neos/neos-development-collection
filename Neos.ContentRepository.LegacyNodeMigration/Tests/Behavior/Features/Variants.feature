Feature: Migrating nodes with content dimensions
  ...
  Scenario: Node variant with different parent node (moved)
    ...
    Then I expect the following events
      ...
      | NodeAggregateWasMoved               | {"nodeAggregateIdentifier": "a1", "nodeMoveMappings": [{"movedNodeOrigin":{"language": "de"},"newParentAssignments":{"69df7cc88c79ca51434f1f69e4f61b08":{"nodeAggregateIdentifier":"b","originDimensionSpacePoint":{"language": "ch"}}},"newSucceedingSiblingAssignments":[]}]} |
  ...
  Scenario: Node variant with different grand parent node (ancestor node was moved) - Note: There is only NodeAggregateWasMoved event for "a" and not for "a1"
    ...
    Then I expect the following events
      ...
      | NodeAggregateWasMoved               | {"nodeAggregateIdentifier": "a", "nodeMoveMappings": [{"movedNodeOrigin":{"language":"de"},"newParentAssignments":{"69df7cc88c79ca51434f1f69e4f61b08":{"nodeAggregateIdentifier":"b","originDimensionSpacePoint":{"language":"ch"}}},"newSucceedingSiblingAssignments":[]}]} |
      | NodeSpecializationVariantWasCreated | {"nodeAggregateIdentifier": "a1", "sourceOrigin": {"language": "de"}, "specializationOrigin": {"language": "ch"}, "specializationCoverage": [{"language": "ch"}]}                                                                                                            |
