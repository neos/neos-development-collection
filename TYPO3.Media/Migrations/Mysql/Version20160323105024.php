 ...
 */
class Version20160323105024 extends AbstractMigration
{
    /**
        ...
        if (count($imageAdjustments) > 0) {
            foreach ($imageAdjustments as $imageAdjustment) {
                ...
                foreach ($supportedConfigurationKeys as $key) {
                    if (isset($configuration[$key]) && !is_null($configuration[$key])) {
                        if (is_bool($configuration[$key])) {
                            $value = (integer) $configuration[$key];
                        } elseif (is_string($configuration[$key])) {
