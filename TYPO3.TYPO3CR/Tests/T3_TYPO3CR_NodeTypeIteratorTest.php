 ...
 */
class T3_TYPO3CR_NodeTypeIteratorTest extends T3_Testing_BaseTestCase {

	public function setUp() {
		$this->mockStorageAccess = $this->getMock('T3_TYPO3CR_StorageAccessInterface');
		$this->mockStorageAccess->expects($this->any())->method('getRawNodeTypeById')->will($this->returnValue(array('name' => 'SuperDuperNodeType')));

		$this->iterator = new T3_TYPO3CR_NodeTypeIterator();
	}
	...
	public function getSizeReturnsCorrectResult() {
		$this->iterator->append(new T3_TYPO3CR_NodeType(1, $this->mockStorageAccess, $this->componentManager));
		$this->iterator->append(new T3_TYPO3CR_NodeType(1, $this->mockStorageAccess, $this->componentManager));
	...
	/**
	 * Tests if a T3_phpCR_NoSuchElementException is thrown when nextNodeType()
