Feature: Translate existing node
  ...
  Scenario: Use a specific target dimension value in a context with fallback languages
    ...
    And I set the node property "title" to "Firma"
    Then I should have a node with path "/sites/typo3cr/company" and value "Firma" for property "title" for the following context:
      | Language      |
      | de_ZZ, mul_ZZ |
    And I should have a node with path "/sites/typo3cr/company" and value "Company" for property "title" for the following context:
      | Language      |
      | en_ZZ, mul_ZZ |
  ...
  Scenario: Setting a property on a node variant not in the best matching language creates a new variant if no explicit target dimension value is set
    ...
    Then I should have a node with path "/sites/typo3cr/company" and value "US company" for property "title" for the following context:
      | Language             |
      | en_US, en_ZZ, mul_ZZ |
    And I should have a node with path "/sites/typo3cr/company" and value "Company" for property "title" for the following context:
      | Language      |
  ...
  Scenario: Setting a property on a node variant not in the best matching language creates no new variant if target dimension value matches
    ...
    Then I should have a node with path "/sites/typo3cr/company" and value "New company" for property "title" for the following context:
      | Language             |
      | en_US, en_ZZ, mul_ZZ |
    And I should have a node with path "/sites/typo3cr/company" and value "New company" for property "title" for the following context:
      | Language      |
      | en_ZZ, mul_ZZ |
  ...
  Scenario: Node variants are created for configured child nodes when materializing a node
    ...
    And I get a node by path "/sites/typo3cr/main" with the following context:
      | Language  |
      | en_US     |
    Then I should have one node
  ...
  Scenario: Node variants are created for configured child nodes when adopting it from another context
    ...
    And I adopt the node to the following context:
      ...
      | de_ZZ, mul_ZZ |
    And I set the node property "title" to "Unternehmen"
    And I get a node by path "/sites/typo3cr/company/main" with the following context:
      | Language      |
      | de_ZZ, mul_ZZ |
    Then I should have one node
